...
var init = exports.init = function(options) {
    ...
    var constructAuthObject = function (Constructor, constructOptions) {
        constructOptions = util._extend({
            url: options.url,
            auth: options.auth
        }, constructOptions);
        return new Constructor(constructOptions);
    };
    ...
    var build_url = function(command) {
        ...
        */
        ...
        var url = util.format.call(this, command, options.url);
    ...
    return {
        build: function(buildOptions, callback) {
            var buildurl;
            buildOptions = util._extend({
                jobname: undefined,
                params: undefined,
                auth: undefined
            }, buildOptions);
            ...
            */
            if (typeof buildOptions.params === 'function') {
                buildurl = build_url(BUILD, buildOptions.jobname);
                callback = buildOptions.params;
            } else {
                buildurl = build_url(BUILDWITHPARAMS+"?"+qs.stringify(buildOptions.params), buildOptions.jobname);
            }
            request({
                method: 'POST',
                url: buildurl,
                ...
                if (response && response.statusCode !== 201 && response.statusCode !== 302) {
                    callback(response, null);
                    return;
                }
                constructAuthObject(QueueItem, {
                    url: response.headers.location // URL of build
                }).notInQueue().then(function (queueItemData) {
                    return constructAuthObject(Build, {
                        url: queueItemData.executable.url
                    });
                }).then(function (jenkinsBuild) {
                    callback(null, jenkinsBuild);
                }, function (err) {
                    callback(err);
                });
            });
